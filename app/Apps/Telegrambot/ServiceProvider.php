...
class ServiceProvider extends BaseAppServiceProvider
{
    ...
    public function render(array $data = [])
    {
        ...
        $viewsPath = resource_path("views/apps/Telegrambot");
        $namespace = strtolower('Telegrambot');
        // Регистрируем namespace, если ещё не зарегистрирован
        View::addNamespace($namespace, $viewsPath);


        // Определяем view (по умолчанию settings)
        $viewName = $data['view'] ?? 'settings';

        // Рендерим Blade-шаблон
        return View::make("{$namespace}::settings", ['settings' => $this->config])->render();
    }
}
